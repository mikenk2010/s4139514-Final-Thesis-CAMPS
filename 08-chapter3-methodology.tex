...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig-camps-architecture}
...
\medskip
Table~\ref{tab:component-roles} summarises how the five components
divide the framework's labour: the first two create and apply the
plurality of perspectives, the third converts plurality into a single
defensible score, and the last two turn residual disagreement into
detection and adaptation. No component alone mitigates the filter;
the fairness properties of CAMPS are a property of the assembly.

\begin{table}[htbp]
\centering
\caption[How each component contributes to CAMPS]{How each component
contributes to CAMPS, and where that contribution is tested.}
\label{tab:component-roles}
\small
\begin{tabular}{p{2.6cm} p{3.4cm} p{5.2cm} c}
\hline
Component & Input $\rightarrow$ Output & Contribution & Tested by \\
\hline
1. Cultural Rubric Interpreter &
$R \rightarrow \{r_1,\dots,r_k\}$ &
Makes cultural perspectives explicit and scholarly-grounded; replaces
pretended neutrality with declared plurality & H2 \\[4pt]
2. Evaluator agent pool &
$(E, r_i) \rightarrow s_i$ &
Independent, deterministic judgments; decorrelates the individual
cultural filters and prevents interaction-driven bias amplification &
H1, H2 \\[4pt]
3. Consensus aggregation &
$\{s_i\}, w \rightarrow S^{*}$ &
Partially cancels individual filters through weighted averaging;
weight vector governs the fairness--accuracy trade-off & H2, H4 \\[4pt]
4. Bias Divergence Score &
$\{s_i\} \rightarrow \mathrm{BDS}(E)$ &
Converts residual inter-agent disagreement into a calibrated
detection signal for culturally contested essays & H3 \\[4pt]
5. Calibration loop &
review outcomes $\rightarrow (w, \tau)$ &
Adapts weights and threshold from human feedback without touching
model weights; keeps the system correctable after deployment & H3, H4 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig-sampling-flow}
...
validation set. Inspection of the downloaded archive (v2.1) confirms
its structure: each of the 140 essays carries ratings from 80 raters
(11{,}200 essay-rating records), where each rater scores ten analytic
dimensions on a 0--10 scale, grouped as Language (intelligibility,
complexity, accuracy, fluency), Content (comprehensibility,
logicality, sophistication, purposefulness), and Attitude
(willingness, involvement), together with a holistic score out of 100
and free-text strengths and weaknesses. The archive also provides
per-essay means over the 80 raters for every dimension. For E3, the
per-essay mean of the Content dimensions serves as the primary human
consensus reference for coherence-oriented scoring, with the holistic
mean as a secondary check, and per-essay rating dispersion across the
80 raters provides the human-disagreement benchmark against which BDS
is compared. The GRA essay set also includes Thai, Indonesian,
Philippine, East Asian, and native-English samples, enabling
lens-level validation on rated data. Two further properties of the
archive, established by inspection of the downloaded data, shape the
E3 design. First, all 140 rated essays address the part-time-job
prompt, so the validation set is single-topic; the E3 findings are
accordingly validated within one prompt, with cross-topic behaviour
assessed on the main cohorts. Second, the 80 raters disagree
substantially among themselves: the mean within-essay standard
deviation of the Content-dimension ratings is approximately 1.8 points
on the 0--10 scale. This human baseline matters for interpreting BDS:
disagreement among evaluators, human or artificial, is the normal
condition of essay assessment, and the question E3 asks is whether the
panel's divergence tracks the essays on which \emph{humans} diverge.
...
\textbf{Experiment E1 (scaled replication)} scores every essay in both
cohorts under the single-agent baseline and the pilot's three-agent
panel, on all four models. E1 establishes whether the cohort scoring
gap exists at scale and across model families (H1), and provides the
$k{=}3$ reference against which the extended panel is compared.

\textbf{Experiment E2 (extended architecture)} scores the same essays
under the five-agent panel, with and without the meta-adjudicator, and
sweeps the consensus weights in steps of $0.05$ under the constraint
$\sum_i w_i = 1$. The sweep traces the fairness--accuracy frontier
directly: for each weight vector, per-cohort agreement with ground
truth is recorded, and H4's boundary is located empirically as the
point where further gains for the disadvantaged cohort begin to cost
native-cohort accuracy. The ablation over $k$ separates the
contribution of panel size from that of the adjudicator.

\textbf{Experiment E3 (detection validation)} computes BDS for the 140
GRA essays, which carry eighty independent human ratings each. Human
rating dispersion provides the reference notion of a genuinely
contested essay; the BDS threshold $\tau$ is calibrated on a random
half of the GRA set by receiver operating characteristic analysis
(Youden's $J$) and evaluated on the held-out half, reporting
sensitivity, false-positive rate, and area under the curve. A
supplementary check compares BDS rankings with the research team's
independent re-scoring, conducted blind to flag status.
...
\citep[cf.][]{cheng2025biasfilter}, intervenes on internal
representations and is infeasible for API-only access by construction.
%% NOTE: cite FairSteer directly once its bib entry is verified.
...
\textbf{Primary metric.} Agreement between automated and ground-truth
scores is measured by Quadratic Weighted Kappa,
\begin{equation}
\kappa_w = 1 - \frac{\sum_{i,j} w_{ij} O_{ij}}
                     {\sum_{i,j} w_{ij} E_{ij}},
\qquad w_{ij} = \frac{(i-j)^2}{(R-1)^2},
\end{equation}
where $O$ and $E$ are the observed and expected score matrices and $R$
the number of scale points; $\kappa_w$ is computed separately per
cohort, and the \emph{gap} $\Delta\kappa_w = \kappa_w^{\mathrm{native}}
- \kappa_w^{\mathrm{SEA}}$ is the fairness quantity of interest.
...
frontier rather than a single optimum: per-cohort $\kappa_w$ at each
weight vector, from which the H4 boundary is read off. Where a single
operating point is needed for comparison, weights are chosen to
maximise mean $\kappa_w$ subject to $\Delta\kappa_w$ not exceeding the
equal-weight value.
