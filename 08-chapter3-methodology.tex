...
scores relative to human judgement, independent of argumentative merit.
\end{definition}
...
scoring of weaker English. It is also what the band-restricted cohort
...
residual quality difference can explain (H1). The second is shrinkage
...
decorrelated (H2). The third is measurable panel disagreement on precisely the
...
truth. This is the mechanism behind the fairness--accuracy boundary
...
score depends on culturally contingent expectations. Such an essay is
...
corpus's reference group). Sampling yields $n \approx 100$ per cohort under a fixed
...
inside the gap. The gap is therefore interpreted as an \emph{upper
...
multi-rater consensus for the validation set. In the GRA archive
...
and a fixed random seed for all sampling. As a result, every result in
...
GRA set (Youden's $J$) and evaluated on the held-out half. Sensitivity,
false-positive rate, and area under the ROC curve are reported.
...
into a single opaque judgement. Bias can then only be asserted away, not
measured, and no divergence signal remains to calibrate a review
...
storage follows the project's data management plan, and no external
participants are recruited. Any future extension involving human
...
falls on a student; BDS exists to route uncertain judgements to a
...
neutral judge. $\Delta$ is therefore an upper bound on filtering. The
treatment effects of interest are \emph{changes} in $\Delta$
...
\textbf{Uncertainty.} Every reported statistic carries its $n$.
Every cohort gap and gap change is reported with a 95\% percentile
bootstrap confidence interval (10{,}000 resamples, fixed seed);
band-agreement and detection statistics are reported as point
estimates with their $n$.
...
tests of gap change (one-sided probability of no reduction). No
...
are recorded. The H4 boundary is read off the resulting Pareto
frontier.
