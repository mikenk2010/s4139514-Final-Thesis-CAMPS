...
feedback-manager, and reflective-reviser agents (average 21\%
improvement in Quadratic Weighted Kappa, QWK, over direct
prompting). Roundtable Essay Scoring
\citep[RES;][]{jang2025roundtable} consolidates trait-based evaluator
agents through simulated discussion (up to 34.86\% QWK improvement
on the ASAP benchmark, zero-shot). Related designs add debate with retrieval
...
with human judgement across six datasets but also reduced intra-model
...
\emph{what a model says} in open-ended generation. The same paper
also introduces Multi-Agent Vote (MAV), an evaluation protocol in
which several agents classify a response's cultural tendency, with
an explicit no-bias option, and their votes are aggregated; this is
the closest architectural precedent for culturally differentiated
judging. CAMPS differs in applying such judging to essay scoring,
in analysing the resulting scores cohort by cohort, and in treating
disagreement between cultural agents as a candidate human-review
signal rather than a vote to be aggregated away.
...
empirically rather than assumed benign (Hypothesis H4 examines,
exploratorily, how the cohort gap moves with proficiency-band
association under reweighting), and panel behaviour is validated
...
That LLMs carry a Western-centric value orientation is well
documented, although whether it extends to \emph{evaluative}
judgement is exactly what this thesis tests. \citet{tao2024cultural} compared five OpenAI model
...
the WEIRD alignment that this thesis proposes as a distal
contributing cause of evaluative epistemic filtering; Tao et al.\
measure value alignment in responses, not the prevalence of
rhetorical forms in training data, so the link is hypothesised.
...
variation across models. The same group's work on
...
lens prompts are fixed verbatim (Appendix~A), decoding uses
temperature 0, and the panel's behaviour is validated empirically
...
interpretations in Chapter~3, where each Cultural Rubric
Interpretation (CRI) prompt is grounded in
...
Four literatures converge on the problem this thesis addresses, and
...
judgement, and it dissolves inter-agent disagreement rather than
...
Within the scope of the searches described above, no published work
was found at the intersection: culturally-diverse rubric
interpretation, applied to evaluative scoring, with inter-agent
divergence used as a calibrated trigger for human review and tested
against multi-rater human ground truth.
That intersection is where CAMPS operates.

\section*{Chapter summary}
Prior multi-agent essay-scoring work varies function, trait, or
model provider rather than the cultural reading of the rubric; MAV
provides a cultural-judging precedent outside scoring; and the
fairness literature documents L1-linked scoring disparities without
