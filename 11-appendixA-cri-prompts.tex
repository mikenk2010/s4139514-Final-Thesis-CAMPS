...
lens agent, is: ``Score the following essay. Respond with JSON:
\{"score": <band>, "rationale": "<2--3 sentences>"\}'' followed by
the essay text. The IELTS descriptors are referred to by name and
not reproduced in any prompt. The adjudicator's user turn lists the
five evaluators' scores and rationales, then the full essay text,
then the JSON instruction. Both are constructed in
