...
LLMs may acquire culturally uneven evaluative expectations from
training and alignment on corpora thought to over-represent Western,
Educated, Industrialised, Rich, and Democratic (WEIRD) contexts;
empirical work shows that widely used models express cultural values
closely aligned with English-speaking and Protestant-European
countries \citep{tao2024cultural}, which motivates this concern
without establishing the prevalence of particular rhetorical
structures in their training data.
...
model. Figure~\ref{fig:rhetoric-shapes} sketches the two shapes.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig-rhetoric-shapes}
\caption[Two valid shapes of argument]{Two valid shapes of argument.
The writer-responsible, thesis-first pattern is hypothesised to
dominate the text LLM evaluators learn from; the corpus composition
itself is not measured here, and the measured Western value
alignment of these models \citep{tao2024cultural} motivates rather
than establishes it. The reader-responsible,
context-first pattern common in many Asian academic traditions
\citep{hinds1987reader, connor2011intercultural} is equally valid
and is hypothesised to be the less familiar of the two.}
\label{fig:rhetoric-shapes}
\end{figure}
...
phase of this research (Part~A) reported scoring disparities between
Southeast Asian (SEA) and native-English cohorts on a 30-essay sample
across two frontier models, and that a culturally-diversified scoring
panel reduced them. The pilot's corpus did not support a controlled
comparison, so Part~B rebuilds the data (Section~\ref{sec:data});
its findings motivated, but do not establish, the claims this thesis
investigates at scale.

The problem this thesis addresses can therefore be stated plainly.
If LLM-based essay scoring applies culturally partial standards,
it does so invisibly and at scale, to students whose academic futures
depend on the result. Scoring disparities against non-native writers
are documented, but no validated framework exists to mitigate them in
evaluative settings, and no calibrated mechanism exists to flag the
essays affected. This thesis measures the native$-$SEA scoring
disparity across models, builds and tests such a framework, and
tests its flagging mechanism against human ground truth. Throughout,
\emph{disparity} or \emph{gap} names what is measured (a difference
in mean score between cohorts) and \emph{filtering} names the
mechanism hypothesised to produce part of it
(Table~\ref{tab:key-terms}).
...
\begin{description}
  ...
  \item[RQ2.] Can a culturally-aware multi-agent scoring architecture
        ...
        flagging for human review the essays it cannot score
        with confidence?
\end{description}
...
\begin{itemize}
  \item \textbf{H1 (Generalisation).} A native$-$SEA cohort score
        gap, consistent with evaluative epistemic filtering, has a
        95\% confidence interval excluding zero on at least four
        LLMs, including an open-weight model.
...
Comparable essay-level human ratings are unavailable for the full
cohort sample, so H4 is examined on a proxy, association with the
corpus's proficiency bands (Section~\ref{sec:metrics}), and is
...
scope (four model families) and the test (a confidence interval on
the cohort scoring gap that excludes zero). RQ2 is a question about the \emph{solution},
...
Mekong, or Neutral). ``Culturally calibrated'' means informed by
that scholarship, not empirically tuned. \\[2pt]
...
in mean score between the two cohorts. It is an \emph{unadjusted
disparity}: the cohorts were not matched on human-judged quality, so
the gap may contain both filtering and real quality differences. \\[2pt]
Contested essay & In E3, an essay in the top quartile of disagreement
among 80 human raters. This is a proxy for the cultural
contestation BDS is meant to detect, not the same thing. \\[2pt]
Band & The 1--9 score an evaluator assigns (integer, with occasional
half-bands); the corpus's own proficiency labels are CEFR bands
(B1\_2, B2+). \\
\hline
\end{tabular}
\end{table}
...
The significance of this work is threefold. First, equity: if
LLM-based assessment comes to gate scholarships, university
admission, and migration pathways, as its adoption in essay scoring
makes plausible, a systematic penalty against non-Western
...
operators. Measuring the disparity rigorously is a precondition for
...
\begin{enumerate}
  ...
  \item \textbf{Architectural:} it develops CAMPS (Culturally-Aware
        Multi-Perspective Scoring), a culturally-calibrated scoring
        panel with a divergence-based review trigger, extending
        multi-perspective bias reduction from generation to
        evaluation, and tests whether scaling it to five lenses and
        an adjudicator helps.
  \item \textbf{Empirical:} it evaluates the framework on 200
        topic-controlled essays from the ICNALE corpus
        \citep{ishikawa2023icnale} across four frontier and open-weight
        models, establishing the disparity at scale on a corpus
        suited to the comparison and characterising where the
        framework changes it.
  \item \textbf{Methodological:} it introduces ROC calibration of
        the Bias Divergence Score against multi-rater human
        disagreement, replacing the pilot's hand-set threshold.
  \item \textbf{Practical:} it maps the gap against a
        proficiency-band association proxy under every panel
        weighting, giving deploying institutions a re-runnable audit
        and an explicit account of what narrowing the gap did and did
        not cost on this evidence.
\end{enumerate}
